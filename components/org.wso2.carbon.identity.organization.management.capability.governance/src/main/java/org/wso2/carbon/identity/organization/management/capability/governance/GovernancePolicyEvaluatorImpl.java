 ...
 * a resource-level policy (if resourceId is provided) is checked first; then an org-level
 * policy is checked. The first matching policy determines the result.</p>
 *
...
public class GovernancePolicyEvaluatorImpl implements GovernancePolicyEvaluator {
    ...
    @Override
    public boolean evaluate(String orgId, String capability, String resourceType, String resourceId)
            throws GovernancePolicyMgtException {

        return evaluateInternal(orgId, capability, resourceType, resourceId);
    }
    ...
    public boolean evaluate(String orgId, String capability, String resourceType)
        ...
        return evaluateInternal(orgId, capability, resourceType, null);
    }

    private boolean evaluateInternal(String orgId, String capability, String resourceType, String resourceId)
            throws GovernancePolicyMgtException {
        ...
        for (int i = 1; i < ancestors.size(); i++) {
            ...
            if (resourceId != null) {
                Optional<ResourceGovernancePolicy> rp = GOVERNANCE_POLICY_DAO.findResourceGovernancePolicy(
                        ancestorId, capability, resourceType, resourceId);
                if (rp.isPresent() && GovernancePolicyServiceImpl.coversOrg(rp.get(), orgId, isDirectChild)) {
                    return !PolicyType.DENY.equals(rp.get().getPolicyType());
                }
            }

            Optional<OrgGovernancePolicy> op = GOVERNANCE_POLICY_DAO.findOrgGovernancePolicy(
                    ancestorId, capability, resourceType);
            if (op.isPresent() && GovernancePolicyServiceImpl.coversOrg(op.get(), orgId, isDirectChild)) {
                return !PolicyType.DENY.equals(op.get().getPolicyType());
            }
        }
