...
public class OrganizationSessionHandlerTest {
    ...
    public void testHandleOrgSessionTermination(String idpName, String authenticatorName, boolean isSessionTerminated)
            throws Exception {
        ...
        Event event = new Event(SESSION_TERMINATE.name(), eventProperties);
        ...
        when(userSessionManagementService.terminateSessionBySessionId(anyString(), anyString())).thenReturn(true);
        ...
        if (isSessionTerminated) {
            verify(userSessionManagementService, times(1)).terminateSessionBySessionId(anyString(), anyString());
        } else {
            verify(userSessionManagementService, never()).terminateSessionBySessionId(anyString(), anyString());
        }
    }

    @Test(dataProvider = "sessionExtendDataProvider")
    ...
    @DataProvider(name = "sessionExtendDataProvider")
    public Object[][] sessionExtendDataProvider() {
    ...
    public Object[][] sessionTerminationDataProvider() {
        ...
        return new Object[][]{{"dummyIdp", ORGANIZATION_AUTHENTICATOR, false},
                {ORGANIZATION_LOGIN_IDP_NAME, "dummyAuthenticator", false},
                {ORGANIZATION_LOGIN_IDP_NAME, ORGANIZATION_AUTHENTICATOR, true}};
    }
}
